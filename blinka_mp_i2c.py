from i2c_mp_usb import I2C_MP_USB as MPSMBus
from adafruit_blinka import Lockable

class SMBus(MPSMBus):
    def write_bytes(self, addr, data):
        self.write_i2c_block_raw(addr, data)
    
    def read_bytes(self, addr, number):
        return self.read_i2c_block_raw(addr, number)

class BlinkaMpI2C(Lockable):
    """I2C class"""
    ...
    def __init__(self, bus_num=None, mode=MASTER, baudrate=None):
        if mode != self.MASTER:
